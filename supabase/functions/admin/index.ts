...
// Make sure the /users endpoint is properly configured in the edge function
serve(async (req) => {
  ...
  try {
    ...
    if (pathname.endsWith("/users") && req.method === "GET") {
      console.log("Processing GET /users endpoint request");
      try {
      ...
      console.log(`Found ${users?.length || 0} users from users table`);
